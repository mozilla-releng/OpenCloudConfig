...
if not exist C:\generic-worker\gw.config cat C:\generic-worker\generic-worker.config | jq ".  | .workerId=\"%worker_id%\" | .publicIP=\"%public_ip%\" | .clientId=\"%client_id%\" | .accessToken=\"%access_token%\"" > C:\generic-worker\gw.config
