...
function Get-NetAdapterLegacy {
  param (
    ...
    [hashtable] $wmiArgs = @{
      ...
      'Filter' = $(
        if ($name.Contains('*')) {
          ("NetConnectionID LIKE '{0}'" -f $name.Replace('*', '%'))
        } else {
          ("NetConnectionID = '{0}'" -f $name)
        }
      );
    },
    [hashtable[]] $wmiProps = @(
...
if(-not $(Get-Command -Name 'Get-NetAdapter' -ErrorAction SilentlyContinue)) {
  New-Alias -Name 'Get-NetAdapter' -Value 'Get-NetAdapterLegacy' -Force
}

function Get-DnsClientServerAddressLegacy {
  param (
    [string] $interfaceAlias,
    [int] $interfaceIndex = (Get-NetAdapter -Name $interfaceAlias | Select-Object -ExpandProperty 'ifIndex')
  )
  $adapterConfig = (Get-CIMInstance -ClassName 'Win32_NetworkAdapterConfiguration' -Filter ('InterfaceIndex={0}' -f $interfaceIndex))
  return @(
    # IPv4 values
    [PSCustomObject]@{InterfaceAlias=$interfaceAlias;InterfaceIndex=$interfaceIndex;AddressFamily=2;ServerAddresses=$adapterConfig.DNSServerSearchOrder};
    # IPv6, only here for completeness since we don't support it yet
    [PSCustomObject]@{InterfaceAlias=$interfaceAlias;InterfaceIndex=$interfaceIndex;AddressFamily=23;ServerAddresses=@()};
  )
}

if (-not $(Get-Command -Name 'Get-DnsClientServerAddress' -ErrorAction SilentlyContinue)) {
  New-Alias -Name 'Get-DnsClientServerAddress' -Value 'Get-DnsClientServerAddressLegacy' -Force
}
...
function Set-DnsClientServerAddressLegacy {
  ...
  $adapterConfig = (Get-CIMInstance -ClassName 'Win32_NetworkAdapterConfiguration' -Filter ('InterfaceIndex={0}' -f $interfaceIndex))
  $invocation = Invoke-CimMethod -InputObject $adapterConfig -MethodName 'SetDNSServerSearchOrder' -Arguments $(if ($resetServerAddresses) { @{} } else { @{ DNSServerSearchOrder = $serverAddresses } })
  if ($invocation.ReturnValue -ne 0) {
...
if (-not $(Get-Command -Name 'Set-DnsClientServerAddress' -ErrorAction SilentlyContinue)) {
  New-Alias -Name 'Set-DnsClientServerAddress' -Value 'Set-DnsClientServerAddressLegacy' -Force
}
...
function Set-DnsServerAddresses {
  param (
    ...
    [string[]] $serverAddresses = @('1.1.1.1', '1.0.0.1')
  )
  begin {
    Write-Log -message ('{0} :: begin - {1:o}' -f $($MyInvocation.MyCommand.Name), (Get-Date).ToUniversalTime()) -severity 'DEBUG'
  }
  process {
    switch ($locationType) {
      'Azure' {
        foreach ($networkAdapter in @(Get-NetAdapter)) {
          $currentDnsAdresses @(Get-DnsClientServerAddress -InterfaceIndex $networkAdapter.ifIndex | ? { $_.ServerAddresses } | % { $_.ServerAddresses })
          if (Compare-Object -ReferenceObject $serverAddresses -DifferenceObject $currentDnsAdresses) {
            try {
              Set-DnsClientServerAddress -InterfaceIndex $networkAdapter.ifIndex -ServerAddresses $serverAddresses
              Write-Log -message ('{0} :: changed dns server addresses on network adapter: {1}, from: {2}, to {3}' -f $($MyInvocation.MyCommand.Name), $networkAdapter.Name, [string]::Join(', ', $currentDnsAdresses), [string]::Join(', ', $serverAddresses)) -severity 'INFO'
            } catch {
              Write-Log -message ('{0} :: exception changing dns server addresses on network adapter: {1}, from: {2}, to {3}. {4}' -f $($MyInvocation.MyCommand.Name), $networkAdapter.Name, [string]::Join(', ', $currentDnsAdresses), [string]::Join(', ', $serverAddresses), $_.Exception.Message) -severity 'ERROR'
              if ($_.Exception.InnerException) {
                Write-Log -message ('{0} :: inner exception: {1}' -f $($MyInvocation.MyCommand.Name), $_.Exception.InnerException.Message) -severity 'ERROR';
              }
            }
          } else {
            Write-Log -message ('{0} :: detected dns server addresses on network adapter: {1}, as: {2}' -f $($MyInvocation.MyCommand.Name), $networkAdapter.Name, [string]::Join(', ', $currentDnsAdresses)) -severity 'INFO'
          }
        }
        break
      }
      default {
        # no explicit dns configuration for the platform. log existing config only.
        foreach ($networkAdapter in @(Get-NetAdapter)) {
          $currentDnsAdresses @(Get-DnsClientServerAddress -InterfaceIndex $networkAdapter.ifIndex | ? { $_.ServerAddresses } | % { $_.ServerAddresses })
          Write-Log -message ('{0} :: detected dns server addresses on network adapter: {1}, as: {2}' -f $($MyInvocation.MyCommand.Name), $networkAdapter.Name, [string]::Join(', ', $currentDnsAdresses)) -severity 'INFO'
        }
